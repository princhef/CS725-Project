...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1), nn.ReLU(), #(depth of current layer,depth of next layer,size of filter)
            nn.MaxPool2d(2),                           #reduce size of layer by 2
            nn.Conv2d(16, 32, 3, padding=1), nn.ReLU(), 
            nn.MaxPool2d(2)
        )
        self.fc = nn.Sequential(
            nn.Linear(32*8*8, 128), nn.ReLU(), #(input layer no of nodes,output layer no of nodes)
            nn.Linear(128, 10)
        )
...
criterionA = nn.CrossEntropyLoss(label_smoothing=0.0)     
criterionB = nn.CrossEntropyLoss(label_smoothing=0.1)     
...
def train(model, loader, opt, criterion):
    ...
    for x, y in loader:          #loading (input,output) these will be according to batch size
        ...
        _, preds = out.max(1)                   #(max value,index of max value)
...
def test(model, loader):
    model.eval()        #sets the model to evaluate mode
    correct = 0
    total = 0
    preds_all = []
    labels_all = []
